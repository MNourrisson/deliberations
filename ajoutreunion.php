	...
	function mafunction($liste,$cle,&$montext,$parent){
		if(isset($liste[$cle]) && count($liste[$cle])!=0){
			...
			foreach($liste[$cle] as $k => $v){
				$selected='';
				if(isset($_POST['type']) && $_POST['type']==$k){$selected='selected="selected"';}
		...
		if($test!=$_COOKIE['email'])
		{
			...
			if(isset($_POST['enregistrement']))
			{
				...
				if(!$hasError)
				{
					...
					$r = new Reunion();
					$idr = $r->addReunion($_POST['type'],$dater);
					
					if($idr)
					{
						$msgok = 'Réunion ajoutée.'; 
					}
					else{
...
?>
	<section id="content">
		...
		<form method="post" action=""> 	
			<fieldset>
				<label for="dater">Date * : </label><input type="text" value="<?php if(isset($_POST['dater']) && $_POST['dater']!=''){echo $_POST['dater'];}?>" name="dater" id="dater"/>
