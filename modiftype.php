<?php 
	...
	if(!isset($_COOKIE['email']))
	{
		...
		if($test!=$_COOKIE['email'])
		{
			...
			$infotr= $tr->getInfoTypeReunion($idtr);
			if(isset($_POST['enregistrement']))
			{
				...
				if(!$hasError)
				{
					...
					$parent = $_POST['parent'];	
					$idtr = $tr->upTypeReunion($idtr,$libelle,$parent);
...
?>
	<section id="content">
		<h2>Modifier le type de réunion</h2>
		...
		<form method="post" action=""> 	
			<fieldset>
				<label for="libelle">Libellé * : </label><input type="text" value="<?php if(isset($_POST['libelle']) && $_POST['libelle']!=''){echo $_POST['libelle'];}elseif(!isset($_POST['libelle']) && $infotr['libelle']!=''){echo $infotr['libelle']; }?>" name="libelle" id="libelle"/>
				...
				if(count($listetr)!=0){
					...
					foreach($listetr as $k => $v){
						$selected='';
						if(isset($_POST['parent']) && $_POST['parent']==$v['id_type_reunion']){$selected='selected="selected"';}elseif(!isset($_POST['parent']) && $infotr['parent']==$v['id_type_reunion']){$selected='selected="selected"';}
