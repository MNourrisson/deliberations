	...
	if(!isset($_COOKIE['email']))
	{
		...
		if($test!=$_COOKIE['email'])
		{
			...
			if(isset($_POST['enregistrement']))
			{
				...
				if(!$hasError)
				{
					$axe = addslashes(trim($_POST['axe']));
					$parent = $_POST['parent'];
				
					$niveau = $_POST['niveau'];		
					
					$idaxe = $a->addAxe($axe,$_POST['charte'],$parent,$niveau);
...
?>
	<section id="content">
		...
		<form method="post" action=""> 	
			<fieldset>
				<label for="axe">Nom de l'axe * : </label><input type="text" value="<?php if(isset($_POST['axe']) && $_POST['axe']!=''){echo addslashes($_POST['axe']);}?>" name="axe" id="axe"/>
				...
				if(count($listecharte)!=0){
					...
					foreach($listecharte as $k => $v){
						$selected='';
						if(isset($_POST['charte']) && $_POST['charte']==$v['id_charte']){$selected='selected="selected"';}
				...
				if(count($listeAxe)!=0){
					...
					foreach($listeAxe as $k => $v){
						$selected='';
						if(isset($_POST['parent']) && $_POST['parent']==$v['id_axe']){$selected='selected="selected"';}
						echo '<option value="'.$v['id_axe'].'" '.$selected.' >'.stripslashes($v['libelle']).'</option>';
					}
				}else{
					echo '<option value="0">Aucun choix possible</option>';
				}
				?></select><br/>
				<label for="niveau">Niveau : </label><input type="number" name="niveau" min="1" max="4" value="<?php if(isset($_POST['niveau']) && $_POST['niveau']!=''){echo $_POST['niveau'];}?>"/><br/>
